...
_send_error() {
    ...
    [ -n "$CHAT_ID" ] && send -d "chat_id=$CHAT_ID" \
        --data-urlencode "text=❌ Вечерний итог упал (строка $line, код $code). journalctl -u d-brain-evening" >/dev/null || true
}
...
REPORT=$(cd "$PROJECT_DIR" && /home/myuser/.local/bin/uv run python3 "$PROJECT_DIR/scripts/evening_data.py" 2>/tmp/evening.err)

if [ -z "$(echo "$REPORT" | tr -d '[:space:]')" ]; then
    [ -n "$CHAT_ID" ] && send -d "chat_id=$CHAT_ID" \
        --data-urlencode "text=❌ Вечерний итог: пустой отчёт. Проверь: cat /tmp/evening.err" >/dev/null || true
    exit 0
fi
