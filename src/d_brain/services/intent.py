...
_CREATE_PATTERNS = [
    r"\b(добав|созда|запиш|запис|постав|внес)\w*\s+" + _TASK_NOUN,
    r"\b(?:задач[уа]|за\s+[у]?дач[уа])[,;:\s]\s*\S",
...
_ACTION_PATTERNS = [
    ...
    # ("перенеси"/"перенести"/"перенесите", "сдвинь"/"сдвинуть", ...)
    r"\b(перенес\w*|передвин\w*|сдвин\w*)\s+.{0,60}\bна\s+",
...
_REMINDER_PATTERNS = [
    r"\bнапомни\w*\b",             # any form of "напомни" is a reminder
