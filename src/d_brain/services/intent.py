...
_CREATE_PATTERNS = [
    r"\b(добав|созда|запиш|запис|постав|внес)\w*\s+(задач\w*|задани\w*|напоминани\w*)\b",
    r"\bзадач[уа][,;:\s]\s*\S",      # "задача: позвонить", "задачу позвонить" (не "задачи")
    r"\bнапомни\w*\s+(мне\s+)?о\b",  # "напомни мне о встрече", "напомните"
]
...
_TRIGGER_PREFIX = re.compile(
    r"^(добав|созда|запиш|запис|постав|внес)\w*\s+(задач\w*|задани\w*|напоминани\w*)[,;:\s]*",
    re.IGNORECASE,
)
_TASK_PREFIX = re.compile(r"^задач[уаие]?[,;:\s]*", re.IGNORECASE)
...
_KNOWN_PROJECTS: dict[str, list[str]] = {
    ...
    "Zapusk International": ["zapusk international", "запуск интернешнл", "запуск интернэшнл", "запуск интернешнел", "запуск интернешенал", "запуск интернационал"],
}
